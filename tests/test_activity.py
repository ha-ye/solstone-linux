...
class TestGetMonitorGeometriesX11:
    ...
    def test_parses_two_connected_monitors(self, monkeypatch):
        monkeypatch.setattr(activity.subprocess, "run", self._make_run(self.XRANDR_TWO_MONITORS))
    ...
    def test_xrandr_nonzero_returns_empty(self, monkeypatch):
        monkeypatch.setattr(activity.subprocess, "run", self._make_run("", returncode=1))
...
class TestIsDpmsActive:
    ...
    async def test_monitor_standby_returns_true(self, monkeypatch):
        monkeypatch.setattr(activity.subprocess, "run", self._make_run(self.XSET_STANDBY))
