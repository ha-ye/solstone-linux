...
from solstone_linux.screencast import Screencaster, X11Screencaster, _match_streams_to_monitors
...
class TestX11Screencaster:
    ...
    async def test_connect_succeeds(self, monkeypatch):
        monkeypatch.setenv("DISPLAY", ":0")
        monkeypatch.setattr(
            "solstone_linux.screencast.shutil.which", lambda _: "/usr/bin/gst-launch-1.0"
        )
        sc = X11Screencaster()
    ...
    async def test_start_no_monitors_raises(self, monkeypatch, tmp_path):
        monkeypatch.setenv("DISPLAY", ":0")
        monkeypatch.setattr(
            "solstone_linux.screencast.X11Screencaster.connect", AsyncMock(return_value=True)
        )
            ...
            with patch(
                ...
                with patch(
                    ...
                    with patch("asyncio.sleep", new=AsyncMock()):
                        streams = await sc.start(str(tmp_path), framerate=1, draw_cursor=False)
